...
WHERE d.BranchID IS NULL
   OR NOT (d.Address    <=> o.Address)
   OR NOT (d.BranchName <=> o.BranchName)
   OR NOT (d.City       <=> o.City)
   OR NOT (d.State      <=> o.State)
   OR NOT (d.Zipcode    <=> o.Zipcode);
...
SELECT
    ...
    CURRENT_DATE,
    CURRENT_TIMESTAMP
FROM odsdb_kumaranm.ods_employees;
...
SELECT
    ...
    CURRENT_DATE,
    CURRENT_TIMESTAMP
FROM odsdb_kumaranm.ods_loans;


# EDW Fact Load
INSERT INTO edwdb_kumaranm.fact_loans (
    LoanID,
    CustomerID,
    BranchID,
    Amount,
...
SELECT distinct
    ...
    CASE 
        WHEN o.Amount >=  70000 THEN 'Y'  #1000000 
        ELSE 'N'
    END AS HighValueFlag,
    CURRENT_DATE,
    CURRENT_TIMESTAMP
FROM odsdb_kumaranm.ods_loans o
LEFT JOIN edwdb_kumaranm.dim_customers c
    ON o.CustomerID = c.CustomerID
LEFT JOIN edwdb_kumaranm.dim_branches b
    ON c.BranchID = b.BranchID
   AND b.is_current = 1;

# EDW Summary Load
INSERT INTO edwdb_kumaranm.fact_loan_summary (
    BranchID,
    RiskIndicator,
    LoanCount,
    TotalLoanAmount,
